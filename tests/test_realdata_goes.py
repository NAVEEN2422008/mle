...
if lc.empty:
    print("[fetch] OFFLINE or endpoint down -> REAL-DATA RUN SKIPPED "
          "(not a failure; suites 1-3 cover synthetic verification)")
    print("=" * 68)
    print("RESULT: SKIPPED (offline)")
    sys.exit(0)
...
#    (feature/detector windows are sample-based -> no resampling needed)
# ------------------------------------------------------------------
df = pd.DataFrame({
